        ...
        public async Task<IActionResult> ConfigurarVenta(int id, int? ventaId)
        {
            ...
            if (ventaId.HasValue)
            {
                ...
                if (venta != null)
                {
                    ...
                    if (montoVenta <= 0)
                    {
                        // Último recurso: usar valores básicos de la venta cuando no hay detalle cargado
                        var baseSubtotal = venta.Subtotal > 0 ? venta.Subtotal : 0;
                        var iva = venta.IVA > 0 ? venta.IVA : baseSubtotal * VentaConstants.IVA_RATE;
                        montoVenta = Math.Max(0, baseSubtotal - venta.Descuento + iva);
                    }
                }
            }
