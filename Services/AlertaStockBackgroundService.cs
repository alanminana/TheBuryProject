...
namespace TheBuryProject.Services
{
    ...
    public class AlertaStockBackgroundService : BackgroundService
    {
        ...
        {
            _logger.LogInformation("AlertaStockBackgroundService iniciado. Ejecutará cada {Intervalo} horas", _intervaloEjecucion.TotalHours);
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                _logger.LogInformation("Próxima verificación de stock en {Minutos} minutos", _intervaloEjecucion.TotalMinutes);
        ...
        private async Task LimpiarAlertasAntiguasAsync()
        {
            try
            {
                using var scope = _serviceProvider.CreateScope();
                ...
                {
                    _logger.LogInformation("Limpieza completada. Se eliminaron {AlertasEliminadas} alertas antiguas", alertasEliminadas);
                }
                else
                {
