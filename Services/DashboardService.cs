    ...
    public class DashboardService : IDashboardService
    {
        ...
        public async Task<DashboardViewModel> GetDashboardDataAsync()
        {
            ...
            var dashboard = new DashboardViewModel
            {
                ...
                ClientesActivos = await _context.Clientes.CountAsync(c => !c.IsDeleted && c.Estado == EstadoCliente.Activo),
                ...
                CantidadVentasHoy = await _context.Ventas
                    .CountAsync(v => !v.IsDeleted && v.FechaVenta.Date == hoy),
                ...
                SaldoPendienteCobro = await _context.Cuotas
                    .Where(c => !c.IsDeleted && c.Estado != EstadoCuota.Pagada && c.Estado != EstadoCuota.Cancelada)
                    .SumAsync(c => c.MontoTotal - c.MontoPagado),

                CuotasVencidasHoy = await _context.Cuotas
                    .CountAsync(c => !c.IsDeleted && c.Estado == EstadoCuota.Vencida && c.FechaVencimiento.Date == hoy),

                CuotasProximasVencer = await _context.Cuotas
                    .CountAsync(c => !c.IsDeleted &&
                               c.Estado == EstadoCuota.Pendiente &&
                               c.FechaVencimiento > hoy &&
                               c.FechaVencimiento <= hoy.AddDays(7)),

                MontoMoraTotal = await _context.Cuotas
                    .Where(c => !c.IsDeleted && c.Estado == EstadoCuota.Vencida)
                    .SumAsync(c => c.MontoPunitorio),

                // KPIs de Cobranza
                CobradoHoy = await _context.Cuotas
                    .Where(c => !c.IsDeleted && c.FechaPago.HasValue && c.FechaPago.Value.Date == hoy)
                    .SumAsync(c => c.MontoPagado),

                CobradoMes = await _context.Cuotas
                    .Where(c => !c.IsDeleted && c.FechaPago.HasValue && c.FechaPago.Value >= inicioMes)
                    .SumAsync(c => c.MontoPagado),

                CuotasCobradas = await _context.Cuotas
                    .CountAsync(c => !c.IsDeleted && c.Estado == EstadoCuota.Pagada),

                // KPIs de Stock
                ProductosTotales = await _context.Productos.CountAsync(p => !p.IsDeleted),
                ProductosBajoStock = await _context.Productos.CountAsync(p => !p.IsDeleted && p.Stock <= p.StockMinimo && p.Stock > 0),
                ProductosSinStock = await _context.Productos.CountAsync(p => !p.IsDeleted && p.Stock == 0),
                ValorInventario = await _context.Productos
                    .Where(p => !p.IsDeleted)
                    .SumAsync(p => p.PrecioCosto * p.Stock),
            ...
            dashboard.TicketPromedio = dashboard.CantidadVentasMes > 0
                ? dashboard.VentasTotalesMes / dashboard.CantidadVentasMes
                : 0;
        ...
        public async Task<List<VentasPorDiaDto>> GetVentasUltimos7DiasAsync()
        {
            ...
            var ventas = await _context.Ventas
                .Where(v => !v.IsDeleted && v.FechaVenta >= fecha7DiasAtras)
        ...
        public async Task<List<VentasPorMesDto>> GetVentasUltimos12MesesAsync()
        {
            ...
            // Llenar meses sin ventas y agregar nombre del mes
        ...
        public async Task<List<ProductoMasVendidoDto>> GetProductosMasVendidosAsync(int top = 10)
        {
            ...
            return await _context.DetallesVenta
                ...
                .Where(d => !d.IsDeleted && d.Venta.FechaVenta >= fecha30DiasAtras)
        ...
        public async Task<List<CreditoPorEstadoDto>> GetCreditosPorEstadoAsync()
        {
            return await _context.Creditos
                .Where(c => !c.IsDeleted)
                .GroupBy(c => c.Estado)
                .Select(g => new CreditoPorEstadoDto
                {
        ...
        public async Task<List<CobranzaPorDiaDto>> GetCobranzaUltimos30DiasAsync()
        {
            ...
            var cobranzas = await _context.Cuotas
                .Where(c => !c.IsDeleted && c.FechaPago.HasValue && c.FechaPago.Value >= fecha30DiasAtras)
                .GroupBy(c => c.FechaPago.Value.Date)
                .Select(g => new CobranzaPorDiaDto
                {
                    Fecha = g.Key,
                    MontoCobrado = g.Sum(c => c.MontoPagado),
                    CuotasCobradas = g.Count()
                })
                .ToListAsync();

            // Llenar días sin cobranza con 0
        ...
        public async Task<List<AlertaDto>> GetAlertasPendientesAsync()
        {
            ...
            var productosStockBajo = await _context.Productos
                .Where(p => !p.IsDeleted && p.Stock <= p.StockMinimo && p.Stock > 0)
                .CountAsync();
            ...
            if (productosStockBajo > 0)
            {
                ...
                {
                    Tipo = "Stock Bajo",
                    Mensaje = $"{productosStockBajo} producto(s) con stock bajo",
                    Prioridad = "Media",
                    Fecha = hoy,
                    Icono = "bi-box-seam",
                    Color = "warning",
            ...
            if (productosSinStock > 0)
            {
                ...
                {
                    Tipo = "Sin Stock",
                    Mensaje = $"{productosSinStock} producto(s) sin stock",
                    Prioridad = "Alta",
                    Fecha = hoy,
            ...
            var cuotasVencidas = await _context.Cuotas
                .CountAsync(c => !c.IsDeleted && c.Estado == EstadoCuota.Vencida);

            if (cuotasVencidas > 0)
            {
                alertas.Add(new AlertaDto
                {
                    Tipo = "Cuotas Vencidas",
                    Mensaje = $"{cuotasVencidas} cuota(s) vencida(s)",
                    Prioridad = "Alta",
                    Fecha = hoy,
                    Icono = "bi-calendar-x",
                    Color = "danger",
            ...
            var cuotasProximasVencer = await _context.Cuotas
                .CountAsync(c => !c.IsDeleted &&
                           c.Estado == EstadoCuota.Pendiente &&
                           c.FechaVencimiento > hoy &&
                           c.FechaVencimiento <= hoy.AddDays(7));
            ...
            if (cuotasProximasVencer > 0)
            {
                alertas.Add(new AlertaDto
                {
                    Tipo = "Próximos Vencimientos",
                    Mensaje = $"{cuotasProximasVencer} cuota(s) vencen en los próximos 7 días",
                    Prioridad = "Media",
                    Fecha = hoy,
                    Icono = "bi-clock",
                    Color = "warning",
            ...
            // Alertas de mora
            var alertasMora = await _context.AlertasCobranza
                .CountAsync(a => !a.IsDeleted && !a.Resuelta && a.Prioridad == 3);

            if (alertasMora > 0)
            {
                alertas.Add(new AlertaDto
                {
                    Tipo = "Alertas Críticas de Mora",
                    Mensaje = $"{alertasMora} alerta(s) crítica(s) de cobranza",
                    Prioridad = "Alta",
                    Fecha = hoy,
        ...
        public async Task<decimal> CalcularTasaMorosidadAsync()
        {
            var totalCuotas = await _context.Cuotas
                .CountAsync(c => !c.IsDeleted && c.Estado != EstadoCuota.Cancelada);
        ...
        public async Task<decimal> CalcularEfectividadCobranzaAsync()
        {
            ...
            var cuotasVencidasMes = await _context.Cuotas
                .Where(c => !c.IsDeleted && c.FechaVencimiento >= inicioMes && c.FechaVencimiento < DateTime.Today)
                .CountAsync();

            if (cuotasVencidasMes == 0)
                return 100;

            var cuotasCobradas = await _context.Cuotas
                .Where(c => !c.IsDeleted &&
