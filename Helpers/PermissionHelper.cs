...
namespace TheBuryProject.Helpers
{
    ...
    public static class PermissionHelper
    {
        ...
        public static bool TienePermiso(this ClaimsPrincipal user, string modulo, string accion)
        {
            ...
            // Construir el claim value esperado (normalizado a lowercase)
            var permisoRequerido = $"{modulo.ToLowerInvariant()}.{accion.ToLowerInvariant()}";

            // Verificar si el usuario tiene el claim de Permission con el valor requerido
            return user.HasClaim(c => 
                c.Type == "Permission" && 
                c.Value.Equals(permisoRequerido, StringComparison.OrdinalIgnoreCase));
        }
